...
%   5 pts — Who cares: Section 1 para 4                    → 5/5
%   5 pts — Data description: Section 1.1                  → 5/5
%
% Approach (15 pts):
%   10 pts — What we did, what's new: Section 2.1          → 9/10
%   5 pts  — Problems encountered: Section 2.2             → 5/5
%   Code repos cited: TGL and TGB cited in Section 2.1     → yes
%
% Experiments and Results (10 pts):
%   10 pts — Measurement, results, analysis: Section 3     → 9/10
%
% Additional (15 pts):
%   5 pts — Figures/tables: 5 figures + 3 tables           → 5/5
%   5 pts — Clarity: self-contained, peer-readable         → 4/5
%   5 pts — DL connections: Section 4 (8 of 9 addressed)   → 5/5
%
% Estimated total: 52-55 / 60
% ============================================================
...
Under TGB evaluation, published TGNN scores drop to $0.14$--$0.40$ MRR, and a parameter-free heuristic called EdgeBank---which simply predicts that previously observed edges will recur---outperforms all three models.

\textbf{Why this matters.}
If a recommender system or fraud detector is built on a model that appears 99\% accurate but actually ranks the correct answer below hundreds of plausible alternatives, deployment decisions are based on a mirage.
For example, a social platform using TGNN-based link recommendation would find that a lookup table of past interactions outperforms the learned model, wasting compute and engineering effort.
...
\begin{enumerate}
\item \textbf{Data converter} (\texttt{tgb\_to\_tgl.py}): Converts TGB's CSV format to TGL's CSR graph format, preserving the bipartite node ID mapping (editors $0$--$8226$, pages $8227$--$9226$) required for correct negative sample lookup.
\item \textbf{EdgeBank baseline}: Reimplemented the unlimited-memory variant from~\cite{poursafaei2022dgb}, which stores all observed $(u,v)$ pairs and predicts recurrence.
\item \textbf{Unified evaluator}: Runs both legacy (1-random-negative AP/AUC) and TGB-official (999-fixed-negative MRR) protocols on the same checkpoint, enabling direct comparison.
\item \textbf{Stratified error analysis}: Decomposes test MRR by edge novelty (recurring vs.\ novel) and source-node degree quartile.
\item \textbf{Component ablations}: Tests the contribution of memory modules and sampling strategies under the hard-negative protocol.
\end{enumerate}

The novelty of this work is in the analysis---we did not modify any model architecture.
...
\textbf{Framework.}
PyTorch 2.2 for autograd and model definition; DGL 2.1 for message-passing graph operations; a custom C++ parallel sampler from TGL for efficient temporal neighborhood sampling.
...
\textbf{Future work.}
Three directions could close the gap to SOTA: (i) \emph{hard-negative-aware training}---training against the same hard negatives used at test time, rather than random negatives; (ii) \emph{hybrid ensembles}---combining EdgeBank's memorization with TGNN's learned representations; (iii) \emph{evaluation under TGB-Seq}---an even harder sequential protocol that prevents information leakage across test edges.
